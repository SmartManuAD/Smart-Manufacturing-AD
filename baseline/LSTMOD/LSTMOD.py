...
from myutils import Utils # Añadido para integrar en ADBench

class LSTMOutlierDetector(CollectiveBaseDetector):

    def __init__(self,seed:int, # añadido 'seed' para repetibilidad
                    contamination=0.1,
                    ...
                    epochs=10,
                    batch_size=8,
                    dropout_rate=0.15, # Defecto = 0.0
                    feature_dim=9,
                    hidden_dim=10, # Defecto = 1
                    n_hidden_layer=1, # Defecto = 0
                    activation=None,
                    diff_group_method='average',
                    model_name='LSTMOutlierDetector' # para integrar en ADBench
                 ):
        ...
        self.seed = seed # añadido 'seed' para repetibilidad
        ...
        # self.model_name = model_name  # para integrar en ADBench
    ...
    def _build_model(self):
        ...
        model_.add(Dense(units=self.feature_dim, input_shape=(self.hidden_dim, 1), activation=None))
        model_.summary() # Añadido para ver la arquitectura del modelo
        model_.compile(loss=self.loss, optimizer=self.optimizer)
        return model_

    def fit(self, X_train: np.array, y_train=None) -> object:
        """ Cambiamos X por X_train e y por y_train para funcionar con ADBench
        ...
        self : object
            Fitted estimator.
        """
        print("XXXX:", X_train.shape)
        X = check_array(X_train).astype(np.float)
    ...
    # Comentamos el método fit original
    ...
    #     """
    #     print("XXXX:", X.shape)
    #     self.utils.set_seed(self.seed) # añadido 'seed' para repetibilidad
    ...
    def decision_function(self, X: np.array): # Nombre original 'decision_function' cambiar a 'predict_score'
        ...
        # se reduce la salida del método sólo a pred_score para permitir integración en ADBench

    def predict_score(self, X): # Añadimos este método por comunalidad con ADBench
